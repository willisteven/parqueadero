...
public class VigilanteServiceImpl implements IVigilanteService {
	...
	public RespuestaJson realizarRegistroVehiculo(Vehiculo vehiculo) {
		try {
		List<Vehiculo> listVehiculos=vehiculoService.buscarPorTipoVehiculoActivo(vehiculo.getIdTipoVehiculo().getTipo(), estado);
		if(this.reglasParqueadero.permiteTipoVehiculo(vehiculo)) {
			if(this.reglasParqueadero.disponibilidadVehiculo(listVehiculos.size(), vehiculo.getIdTipoVehiculo().getTipo())) {
	...
	public RespuestaJson realizarSalida(Date salidaFecha, Vehiculo vehiculo) {
		...
		if(registro!=null) {
			...
			this.calcularCobroParqueadero(vehiculo,registro.getIngresoFecha(),salidaFecha);
				...
				return new RespuestaJson(200,
						"El vehiculo no se encuentra activo, o no ha ingresado al parqueadero.");
			}
		return null;
	}
		...
		public double calcularCobroParqueadero(Vehiculo vehiculo, Date ingresoFecha, Date salidaFecha) {
			...
			Precio precioHora=this.precioService.obtenerPrecioPorTipoVehiculoYTiempo(vehiculo.getIdTipoVehiculo().getIdTipoVehiculo(), 1);
			Precio precioDia=this.precioService.obtenerPrecioPorTipoVehiculoYTiempo(vehiculo.getIdTipoVehiculo().getIdTipoVehiculo(), 2);
